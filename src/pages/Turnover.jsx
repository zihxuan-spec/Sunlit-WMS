  ...
  const handleReturnToInbound = () => {
    ...
    showConfirm(
      ...
      async () => {
        ...
        if (addToPendingDB) await addToPendingDB(barcodes);
        setSelectedIds([]);
        fetchTurnover();
              ...
              return (
                <div key={item.id}
                  ...
                  style={{ padding:'9px 12px', marginBottom:4, borderRadius:6, cursor:'pointer',
                    background: sel ? (document.documentElement.classList.contains('light') ? '#dbeafe' : 'var(--dk-accent-bg)') : surface2,
                    border:`1px solid ${sel ? 'var(--dk-accent)' : 'transparent'}` }}>
                  <div style={{ fontFamily:'monospace', fontSize:12, fontWeight:700,
                    color: sel ? (document.documentElement.classList.contains('light') ? '#1d4ed8' : '#93c5fd') : textPrim }}>
            ...
            {Object.values(completedBatches).map(b => (
              <div key={b.bNo} style={{ padding:'10px 12px', marginBottom:4, borderRadius:6, background: surface2, borderLeft:'3px solid #10b981' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:8 }}>
                  ...
                  <button className="btn btn-success btn-sm" style={{ flexShrink:0, fontSize:11, padding:'5px 10px', minHeight:'unset' }} disabled={isSubmitting}
