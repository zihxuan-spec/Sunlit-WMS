...
export default function MES({ t, lang, currentUser, showAlert }) {
  ...
  const startProduction = async (batch) => {
    ...
    // Resume from correct step based on containers' current_step
    const minStep = contData && contData.length > 0
      ? Math.min(...contData.map(c => c.current_step ?? 1)) - 1
      : 0;
  ...
  const handleSaveAndNext = async () => {
    ...
    // Filling: save weight data in parallel
    if (isFillingStep()) {
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px' }}>
              <div>
                ...
                <div style={{ fontSize: '13px', color: '#666', marginTop: '4px' }}>
                  {t.labelStep} {currentStepIdx + 1} / {steps.length} &nbsp;|&nbsp; {t.labelBatchNo}: {activeBatch.batch_no}
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ marginBottom: '12px' }}>
                  ...
                  {currentPackPallet && (
                    <div style={{ fontSize: '12px', color: '#1976d2', marginTop: '4px' }}>
                      ▶ {lang === 'zh' ? '目前棧板' : 'Active'}: {currentPackPallet} ({currentPalletDrums.length}/{maxPerPallet})
                    </div>
                  )}
                </div>
                ...
                {packingPallets.map((p, i) => (
                  <div key={i} style={{ background: '#f5f5f5', borderRadius: '6px', padding: '8px 12px', marginTop: '8px', fontSize: '13px' }}>
                    ...
                    {p.drums.length >= maxPerPallet && <span style={{ color: '#4caf50', marginLeft: '8px' }}>✅ {lang === 'zh' ? '已滿' : 'Full'}</span>}
