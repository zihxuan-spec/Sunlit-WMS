...
export default function MES({ t, lang, currentUser, showAlert }) {
  ...
  const [batchContainerCounts, setBatchContainerCounts] = useState({});
  ...
  const fetchRules = async () => {
    const [{ data: palletData }, { data: gunData }] = await Promise.all([
  ...
  // getPalletRule: match by material_code field in pallet_barcode_rules
  // Falls back to checking if any rule's material_code matches
  ...
  const handlePackScan = (e) => {
    ...
    const max = rule ? rule.containers_per_pallet : 999;
    // For packaging: accept any barcode that's in containers (child drums)
    if (containers.length > 0 && !containers.find(c => c.barcode === input)) {
      showAlert(t.msgNotInTurnover); return;
    }
  ...
  const handleSaveAndNext = async () => {
    ...
    const rule = getPalletRule(activeBatch?.material_code);
    // Packaging: use pallet mode if rule exists OR if we're in packaging step with containers
    const usePackingMode = isPackagingStep() && (rule || containers.length > 0);

    if (usePackingMode) {
      const totalExpected = containers.length > 0 ? containers.length : packedDrums.length;
      if (packedDrums.length === 0) return showAlert(t.msgVerifyAll + ' (0 drums packed)');
    ...
    // Packaging without containers (edge case)
    if (isPackagingStep() && !usePackingMode) {
      setIsSubmitting(true);
      await supabase.from('production_batches').update({ status: 'completed' }).eq('batch_no', activeBatch.batch_no);
      await supabase.from('turnover_inventory').update({ status: 'completed' }).eq('batch_no', activeBatch.batch_no);
      setCompleteInfo({ batchNo: activeBatch.batch_no, material: activeBatch.material_code, pallets: [], totalDrums: 0 });
      setIsSubmitting(false); setShowComplete(true); setActiveBatch(null); fetchBatches(); return;
    }
  ...
  const maxPerPallet = rule ? rule.containers_per_pallet : null;
  const currentPalletDrums = packingPallets.find(p => p.palletBarcode === currentPackPallet)?.drums || [];
  const usePackingUI = isPackagingStep() && (rule || containers.length > 0);
  ...
  const surface  = 'var(--lt-surface, var(--dk-surface, #1e2535))';
  const surface2 = 'var(--bg-section)';
  const border   = 'var(--border)';
  const textPrim = 'var(--lt-text, var(--dk-text, #e2e8f0))';
  const textMut  = 'var(--lt-text-2, var(--dk-text-3, #64748b))';
              ...
              {batches[status].map(b => (
                <div key={b.batch_no} className="kanban-item"
                  ...
                  <div className="kanban-meta">
                    ...
                    {batchContainerCounts[b.batch_no] > 0 && (
                      <span style={{ marginLeft: 6 }}>· {batchContainerCounts[b.batch_no]} {lang === 'zh' ? '桶' : 'drums'}</span>
                    )}
                    {b.customer && <span style={{ marginLeft: 6 }}>· {b.customer}</span>}
        ...
        <div className="modal-overlay">
          <div className="modal-card" style={{ maxWidth: 760, width:'94%', maxHeight:'92vh', overflowY:'auto', background: surface }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:18 }}>
              <div>
                <h3 style={{ fontSize:18, marginBottom:4, color: textPrim, fontWeight: 600 }}>
                  {steps[currentStepIdx]?.step_name}
                </h3>
            ...
            {isFillingStep() && (
              <>
                  ...
                  <div>
                    ...
                    {gunStations.length > 0 ? (
                      <select value={formData.gunNumber} onChange={e => setFormData(f => ({...f, gunNumber: e.target.value}))}>
                        <option value="">{lang==='zh'?'選擇槍號...':'Select gun station...'}</option>
                        {gunStations.map(g => (
                          <option key={g.id} value={g.code}>{g.label || g.code}</option>
                        ))}
                      </select>
                    ) : (
                ...
                <div style={{ border:`1px solid ${border}`, borderRadius:8, padding:16, marginBottom:16 }}>
                  <div style={{ fontSize:12, color: textMut, marginBottom:10, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <span>{lang==='zh'?'逐桶掃描':'Scan drums one by one'} · <strong style={{ color: textPrim }}>{scannedList.length}/{containers.length}</strong></span>
                    {scannedList.length > 0 && scannedList.length < containers.length && (
                      <span style={{ color:'var(--dk-accent)', fontSize:11 }}>{lang==='zh'?'中斷後已恢復進度':'Progress restored'}</span>
                    )}
                  </div>
                  <form onSubmit={handleVerify}>
                    <input ref={scanRef} value={scanInput} onChange={e => setScanInput(e.target.value.toUpperCase())} placeholder={lang==='zh'?'掃描桶號...':'Scan barcode...'} autoFocus style={{ marginBottom: 8 }} />
                  </form>
                  <div>
                    {containers.map(c => {
                        ...
                        <div key={c.id} style={{ padding:'9px 0', borderBottom:`1px solid ${border}` }}>
                          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                            ...
                            {done && hasWeights && !isActive && (
                              <span style={{ fontSize:11, color: textMut, marginLeft:'auto' }}>
                                {w.empty} / {w.setting} / {w.filling} kg
                              </span>
                            )}
                            {done && (
                              <button style={{ fontSize:11, color:'var(--dk-accent)', background:'none', border:'none', cursor:'pointer', marginLeft: (hasWeights && !isActive) ? 8 : 'auto', padding:'2px 6px' }}
            ...
            {usePackingUI && (
              <div>
                <div style={{ padding:'10px 14px', background:'rgba(59,130,246,.12)', borderRadius:8, marginBottom:14, fontSize:13, color: textPrim, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <span>
                    {lang==='zh'?'進度':'Progress'}: <strong>{packedDrums.length}{containers.length > 0 ? `/${containers.length}` : ''}</strong> {lang==='zh'?'桶':'drums'}
                    {maxPerPallet && <> · Max <strong>{maxPerPallet}</strong>/pallet</>}
                  </span>
                  <span style={{ fontSize:11, color: textMut }}>{lang==='zh'?'掃描桶號加入棧板':'Scan drums into pallets'}</span>
                </div>

                {/* Pallet barcode input */}
                <div style={{ display:'flex', gap:8, marginBottom:12, alignItems:'flex-end' }}>
                  ...
                  <button className="btn btn-primary" style={{ flexShrink:0, marginBottom:0 }} onClick={() => {
                    ...
                    if (packingPallets.find(p => p.palletBarcode === formData.newPallet)) return showAlert(lang==='zh'?'此棧板條碼已使用':'Pallet barcode already used');
                    setCurrentPackPallet(formData.newPallet.trim());
                  }}>{lang==='zh'?'設定':'Set pallet'}</button>
                </div>
                ...
                {currentPackPallet && (
                  <div style={{ padding:'8px 12px', background: surface2, borderRadius:6, marginBottom:12, fontSize:12, color:'var(--dk-accent)', display:'flex', alignItems:'center', gap:8 }}>
                    ...
                    <span style={{ color: textMut }}>— {currentPalletDrums.length}{maxPerPallet ? `/${maxPerPallet}` : ''} drums</span>
                    {maxPerPallet && currentPalletDrums.length >= maxPerPallet && <span style={{ color:'#10b981' }}>Full</span>}
                  </div>
                )}
                ...
                <div style={{ border:`2px solid var(--dk-accent)`, borderRadius:8, padding:14, marginBottom:14 }}>
                  <div style={{ fontSize:12, color: textMut, marginBottom:8, fontWeight:600 }}>
                    {lang==='zh'?'掃描桶號加入目前棧板：':'Scan drum barcode into current pallet:'}
                ...
                {packingPallets.map((p,i) => (
                  <div key={i} style={{ padding:'9px 12px', background: surface2, borderRadius:6, marginBottom:6, fontSize:12 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                      ...
                      {maxPerPallet && p.drums.length >= maxPerPallet && <span style={{ color:'#10b981', fontSize:11 }}>Full</span>}
                    </div>
            ...
            {!isFillingStep() && !usePackingUI && (
              <div style={{ border:`1px solid ${border}`, borderRadius:8, padding:16, marginBottom:16 }}>
                ...
                <form onSubmit={handleVerify}>
                  <input ref={scanRef} value={scanInput} onChange={e => setScanInput(e.target.value.toUpperCase())}
                    autoFocus placeholder={lang==='zh'?'掃描桶號驗證...':'Scan barcode to verify...'} style={{ marginBottom: 8 }} />
                </form>
                <div>
                  {containers.map(c => (
                    <div key={c.id} style={{ padding:'8px 0', borderBottom:`1px solid ${border}`, fontFamily:'monospace', fontSize:13,
        ...
        <div className="modal-overlay">
          <div className="modal-card" style={{ maxWidth:460, textAlign:'center', background: surface, border:`1px solid ${border}` }}>
            ...
            <div style={{ background: surface2, borderRadius:8, padding:16, margin:'12px 0', textAlign:'left' }}>
              ...
              <div style={{ fontSize:13, marginBottom:6 }}>
                <span style={{ color: textMut }}>{lang==='zh'?'物料':'Material'}: </span>
                <span className="badge badge-gray">{completeInfo.material}</span>
              </div>
              <div style={{ fontSize:13, marginBottom:6 }}>
                <span style={{ color: textMut }}>{lang==='zh'?'總桶數':'Total drums'}: </span>
                <strong style={{ color: textPrim }}>{completeInfo.totalDrums}</strong>
              </div>
              {completeInfo.pallets.map((p,i) => (
                <div key={i} style={{ fontSize:12, color: textMut, marginTop:4 }}>
