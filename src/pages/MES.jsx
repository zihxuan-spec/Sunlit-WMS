...
export default function MES({ t, lang, currentUser, showAlert }) {
  ...
  const handleSaveAndNext = async () => {
    ...
    if (usePackingUI) {
      ...
      setIsSubmitting(false); setShowComplete(true); setActiveBatch(null); setActiveBatchCT(null);
      // Clear scanning/packing state so stale data doesn't flash when the next batch is opened
      setPackingPallets([]); setCurrentPackPallet(''); setPackedDrums([]);
      setScannedList([]); setWeightData({}); setActiveFillDrum(null);
      fetchBatches(); return;
    }
