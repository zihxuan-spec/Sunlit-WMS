...
const PAGE_SIZE = 50;
export default function SparePart({ lang, currentUser, currentUid, userRole, showAlert, showConfirm }) {
  ...
  useEffect(()=>{
    ...
    // Unique channel name — avoids cross-tab collisions and dept-switch ghost subscriptions
    const channelName = `sp-rt-${currentUid||'anon'}-${dept}`;
  ...
  const resolveRow=async(idx,pn)=>{
    if(!pn) return;
    // Check part belongs to user's department
    if(dept !== 'All'){
    ...
    const{data}=await applyDept(supabase.from('view_sp_inventory').select('*').eq('part_number',pn)).maybeSingle();
  ...
  const submitTx=async()=>{
    ...
    if(dept !== 'All'){
      ...
      if(invalid.length>0){
        const invalidPns = invalid.map(i=>i.id).join(', ');
        showAlert(lang==='zh'?`料號 ${invalidPns} 不屬於 ${dept} 部門，無法操作`:`Part(s) ${invalidPns} not assigned to ${dept} department`);
        return;
      }
    }
  ...
  const deleteMaster=(item)=>showConfirm(L(`Delete "${item.part_number}"?`,`確定刪除「${item.part_number}」？`),async()=>{
    ...
    if(totalStock>0){
      ...
      showAlert(L(`Cannot delete — has stock (${totalStock}): ${breakdown}`,`尚有庫存 (${totalStock})，無法刪除：${breakdown}`));
      return;
    }
  ...
  const exportExcel=async(type)=>{
    ...
    // CSV field escaping: wrap in quotes, escape internal quotes by doubling, handle null/undefined
    const esc=(v)=>{
      if(v==null) return '""';
    ...
    const rows=[headers.map(esc),...data.map(r=>headers.map(h=>esc(r[h])))];
