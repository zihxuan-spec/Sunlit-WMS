...
export default function Outbound({
  ...
  useEffect(() => {
    ...
    fetchCustomers();
    // Listen for realtime changes so Admin-side customer edits reflect here without reload
    const ch = supabase.channel('outbound-customers')
              ...
              <div key={shelf.id} style={{
                ...
                <div onClick={() => setOutboundPending(p => p.filter(x => x.id !== shelf.id))}
                  style={{
                    ...
                    fontSize: 12, fontWeight: 700, transition: 'background .1s, color .1s',
                  }}
