...
export default function Login({ onLogin, t, lang, setLang, showAlert }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (data.pin && data.pin !== pw) return showAlert(lang === 'zh' ? '密碼錯誤' : 'Incorrect password');
  ...
  return (
    <div style={{
      ...
      background: isLight ? '#f3f4f6' : '#0f1623',
      padding: 24,
    }}>
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          <div style={{ fontSize: 12, color: isLight ? '#6b7280' : '#64748b', letterSpacing: '.5px' }}>
            Warehouse · MES · Production
          </div>
        </div>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ position: 'relative' }}>
              <input
                ...
                style={{
                  width: '100%', padding: '11px 48px 11px 14px', fontSize: 14,
                  border: `1.5px solid ${isLight ? '#d1d5db' : '#334155'}`,
                  borderRadius: 8, outline: 'none',
                  background: isLight ? '#f9fafb' : '#0f1623',
                  color: isLight ? '#111827' : '#f1f5f9',
