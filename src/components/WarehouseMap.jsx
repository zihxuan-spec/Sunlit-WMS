  ...
  return (
    <div className="card">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '10px', borderBottom: '2px solid #ccc', paddingBottom: '5px', marginBottom: '15px' }}>
        <div className="warehouse-controls" style={{ borderBottom: 'none', margin: 0, padding: 0 }}>
          {currentView !== 'outbound' && (
            <button className={`wh-tab ${activeWarehouse === 'North Warehouse' ? 'active' : ''}`} onClick={() => setActiveWarehouse('North Warehouse')}>🟦 {lang==='zh'?'北邊倉庫':'North WH'}</button>
          )}
          {currentView !== 'inbound' && (
            <button className={`wh-tab ${activeWarehouse === 'South Warehouse' ? 'active' : ''}`} onClick={() => setActiveWarehouse('South Warehouse')}>🟧 {lang==='zh'?'南邊倉庫':'South WH'}</button>
          )}
        </div>
      ...
      <div className="warehouse-controls" style={{marginTop:'8px'}}>
        {zones.map(z => <button key={z} className={`zone-tab ${activeZone === z ? 'active' : ''}`} onClick={() => setActiveZone(z)}>{z} {lang==='zh'?'區':'Zone'}</button>)}
      </div>
        ...
        {filteredShelves.map(shelf => {
          ...
          return (
            <div key={shelf.id} className={classNames} onClick={() => handleLocalShelfClick(shelf)}
              style={{ gridColumn: shelf.col_idx, gridRow: shelf.row_idx, className: isEmpty ? 'shelf shelf-empty' : 'shelf shelf-occupied', color: textColor, cursor: isMapClickable ? 'pointer' : 'default', opacity: opacity }}>
