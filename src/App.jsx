...
export default function App() {
  ...
  const [currentUid, setCurrentUid] = useState(null); // auth.user.id — stable ID for DB filtering
  ...
  // ── Auto logout: idle 30min OR absolute session 8hr ────────────
  // Idle: user inactive for 30min — safety for shared terminals
  // Absolute: session max duration — bounded even if user keeps poking the screen
  const IDLE_LIMIT = 30 * 60 * 1000;        // 30 minutes idle
  const MAX_LIMIT  = 8 * 60 * 60 * 1000;    // 8 hours absolute (one shift)
  ...
  const fetchInboundQueue = useCallback(async () => {
    ...
    const { data } = await supabase.from('inbound_queue').select('*').eq('added_by_uid', currentUid).order('created_at', { ascending: true });
    if (data) setPendingItemsState(data.map(r => ({ id: r.id, barcode: r.barcode })));
  }, [currentUid]);
  const addToPendingDB = async (barcodes) => {
    if (!barcodes.length) return;
    // Store both uid (stable identity) and name (human-readable audit trail)
    const { data } = await supabase.from('inbound_queue').insert(
      barcodes.map(bc => ({ barcode: bc, added_by: currentUser, added_by_uid: currentUid }))
    ).select();
  ...
  // Write audit history — log failures to console so we notice missing audit entries
  const writeHistory = async (records) => {
    const { error } = await supabase.from('shelf_history').insert(records);
    if (error) console.error('[WMS] writeHistory failed:', error, 'records:', records);
  };
  ...
  // ── handleSmartScan: declared BEFORE any useEffect that references it ──
  // langRef/tRef let handleSmartScan read current lang without being rebuilt on every lang change
  const langRef = useRef(lang);
  const tRef = useRef(t);
  useEffect(() => { langRef.current = lang; tRef.current = t; }, [lang, t]);

  const handleSmartScan = useCallback((bc) => {
    if (viewRef.current === 'outbound') {
      ...
      if (match.length > 0) { setOutboundPending(prev => [...prev, ...match].sort((a,b) => a.zone!==b.zone?a.zone.localeCompare(b.zone):a.row_idx-b.row_idx)); setActiveZone(match[0].zone); }
        ...
        if (nm.length) { setOutboundNotFound(p=>p.includes(bc)?p:[...p,bc]); showAlert(langRef.current==='zh'?`此批號在北倉 ${nm[0].id}，請先移至南倉再出貨。`:`Barcode found in North WH at ${nm[0].id}. Move to South WH first.`); }
      ...
      if (ex.includes(bc)) return showAlert(tRef.current.msgDupSingle.replace('{bc}',bc));
    ...
    // showAlert and addToPendingDB are closures over state but use refs internally;
    // we intentionally keep deps minimal so handleSmartScan's identity is stable for listeners.
  ...
  useEffect(() => {
    ...
    // Unique channel name per user session to avoid collisions when multiple tabs are open
    const channelName = currentUid ? `wms-main-${currentUid}` : 'wms-main';
  ...
  }, [currentUser, currentUid, fetchShelves, fetchTurnover, fetchContainerTypes, fetchInboundQueue, fetchOutboundAssignQueue, handleSmartScan]);
  ...
  useEffect(() => {
    ...
    const onKey=(e)=>{
      ...
      if(e.key.length===1&&/[a-zA-Z0-9\-]/.test(e.key)){buf+=e.key;clearTimeout(tmr);tmr=setTimeout(()=>{buf='';},120);}
    };
    window.addEventListener('keydown',onKey);
    return ()=>window.removeEventListener('keydown',onKey);
  }, [currentView, currentUser, modal.isOpen, handleSmartScan]);
  ...
  const handleShelfClickInbound = async (shelf) => {
    if (shelf === 'transfer_all') {
      ...
      showConfirm(t.msgTransferTurnoverConfirm.replace('{n}', inboundTransferSelected.length), async () => {
        const selected = inboundTransferSelected;
        // Empty shelves in parallel with error tracking
        ...
        const succeeded = selected.filter((_, i) => results[i].status === 'fulfilled' && !results[i].value?.error);
        const failed = selected.filter((_, i) => results[i].status === 'rejected' || results[i].value?.error);

        if (succeeded.length) {
          await writeHistory(succeeded.map(s=>({shelf_id:s.id,action:'outbound_turnover',product_barcode:s.product_barcode,operator:currentUser})));
          const { error: insertErr } = await supabase.from('turnover_inventory').insert(
            succeeded.map(s=>({product_barcode:s.product_barcode,batch_date:s.batch_date,added_by:currentUser}))
          );
          if (insertErr) {
            showAlert(lang==='zh'
              ? `貨架已清空但 ${succeeded.length} 筆轉入週轉區失敗：${insertErr.message}`
              : `Shelves cleared but ${succeeded.length} turnover inserts failed: ${insertErr.message}`);
          }
          fetchTurnover();
          beep();
        }
        ...
        setInboundTransferSelected(failed);

        if (!failed.length) {
          showAlert(t.msgAutoSuccess);
        } else if (!succeeded.length) {
          showAlert(lang==='zh' ? `全部 ${failed.length} 筆轉移失敗，請重試` : `All ${failed.length} transfers failed. Please retry.`);
        } else {
          showAlert(lang==='zh'
            ? `${succeeded.length} 筆成功，${failed.length} 筆失敗仍保留選取`
            : `${succeeded.length} succeeded, ${failed.length} failed and kept selected for retry`);
        }
      });
      return;
    }
  ...
  const handleAutoAssignOutbound = async (zoneStr) => {
    ...
    showConfirm(t.msgAutoConfirm.replace('{n}',n).replace('{z}',zoneStr), async () => {
      const results = await Promise.allSettled(toAssign.map((obj,i) =>
        supabase.from('shelves').update({status:'occupied',product_barcode:obj.barcode,batch_no:obj.batch_no,batch_date:today,last_updated_by:currentUser}).eq('id',empty[i].id)
      ));
      const succeeded = toAssign.filter((_,i) => results[i].status==='fulfilled' && !results[i].value?.error);
      const failed    = toAssign.filter((_,i) => results[i].status==='rejected' || results[i].value?.error);
      if (succeeded.length) {
        await writeHistory(succeeded.map((obj) => {
          const origIdx = toAssign.indexOf(obj);
          return {shelf_id:empty[origIdx].id,action:'inbound',product_barcode:obj.barcode,batch_no:obj.batch_no,operator:currentUser,batch_date:today};
        }));
        await removeFromOutboundAssignDB(succeeded.map(i=>i.id));
        beep();
      }
      setSelectedOutboundAssign(null);
      if (!failed.length) showAlert(t.msgAutoSuccess);
      else showAlert(lang==='zh'
        ? `${succeeded.length} 筆成功，${failed.length} 筆失敗`
  ...
  const handlePickAllFound = async (customerName='') => {
    ...
    // Use allSettled so a partial failure doesn't abandon the rest silently
    const shelfUpdates = await Promise.allSettled(outboundPending.map(s =>
      supabase.from('shelves').update({status:'empty',product_barcode:null,batch_no:null,batch_date:null,last_updated_by:currentUser}).eq('id',s.id)
    ));
    const failedShelves = outboundPending.filter((_,i) =>
      shelfUpdates[i].status === 'rejected' || shelfUpdates[i].value?.error
    );
    const succeeded = outboundPending.filter((_,i) =>
      shelfUpdates[i].status === 'fulfilled' && !shelfUpdates[i].value?.error
    );

    // Only write history / update tracking for shelves that actually cleared
    if (succeeded.length) {
      await writeHistory(succeeded.map(s=>({shelf_id:s.id,action:'outbound_customer',product_barcode:s.product_barcode,batch_no:s.batch_no,operator:currentUser})));
      const recyclable=succeeded.filter(s=>getContainerTypeByBarcode(s.product_barcode)?.is_reusable);
      ...
      const batchNos=[...new Set(succeeded.map(s=>s.batch_no).filter(Boolean))]; const shippedAt=new Date().toISOString();
      if(batchNos.length){
    ...
    // Keep failed ones in the pending list so user can retry; drop the ones that shipped
    setOutboundPending(failedShelves);
    fetchTurnover();
    if (failedShelves.length === 0) {
      showAlert(t.msgAutoSuccess);
    } else if (succeeded.length === 0) {
      showAlert(lang==='zh'
        ? `全部 ${failedShelves.length} 筆出貨失敗，請重試`
        : `All ${failedShelves.length} shipments failed. Please retry.`);
    } else {
      showAlert(lang==='zh'
        ? `${succeeded.length} 筆出貨成功，${failedShelves.length} 筆失敗仍保留在清單中`
        : `${succeeded.length} shipped, ${failedShelves.length} failed and kept in list for retry`);
    }
  };
  ...
  return (
    <div className="app-shell">
      ...
      <div className="main-content">
        ...
        {currentView==='sparepart'&&['Admin','QC','Facility','Production'].includes(userRole)&&<SparePart lang={lang} currentUser={currentUser} currentUid={currentUid} userRole={userRole} showAlert={showAlert} showConfirm={showConfirm}/>}
