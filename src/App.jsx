...
export default function App() {
  ...
  const fetchInboundQueue = useCallback(async () => {
    const { data } = await supabase.from('inbound_queue').select('*').order('created_at', { ascending: true });
  ...
  const handlePickAllFound = async (customerName = '') => {
    ...
    if (recycledRules.length) {
      const recyclable = outboundPending.filter(s => s.product_barcode && recycledRules.find(r => s.product_barcode.startsWith(r.prefix)));
