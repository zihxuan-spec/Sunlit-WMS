...
export default function App() {
  ...
  const handlePickAllFound = async (customerName = '') => {
    ...
    // Atomic shipment via RPC (prevents race conditions)
    const shelfIds = outboundPending.map(s => s.id);
    const batchNos = [...new Set(outboundPending.map(s => s.batch_no).filter(Boolean))];
    const { error: shipError } = await supabase.rpc('ship_outbound', {
      p_shelf_ids: shelfIds,
      p_batch_nos: batchNos,
      p_customer:  customerName,
      p_operator:  currentUser,
    });
    if (shipError) return showAlert(lang === 'zh' ? `出貨失敗：${shipError.message}` : `Shipment failed: ${shipError.message}`);
