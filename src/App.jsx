...
export default function App() {
  ...
  // Refs for keyboard/socket events
  ...
  useEffect(() => {
    if (shelves.length > 0) {
      const zones = Array.from(new Set(shelves.filter(s => s.warehouse === activeWarehouse).map(s => s.zone))).sort();
  ...
  const fetchRecycledRules = async () => {
  ...
  const handleSmartScan = (barcodeToSearch) => {
    if (viewRef.current === 'outbound') {
      ...
      const existingBarcodes = [
        ...
        ...turnoverRef.current.map(t => t.product_barcode),
  ...
  const handleAddPendingInbound = (batchInput) => {
    ...
    const existingBarcodes = [
      ...
      ...turnoverItems.map(t => t.product_barcode),
      ...pendingItems,
  ...
  const handleAutoAssign = async (zoneStr) => {
    ...
    showConfirm(t.msgAutoConfirm.replace('{n}', assignCount).replace('{z}', zoneStr), async () => {
      await Promise.all(itemsToAssign.map((bc, idx) =>
        supabase.from('shelves').update({ status: 'occupied', product_barcode: bc, batch_date: todayDate, last_updated_by: currentUser }).eq('id', emptyShelves[idx].id)
      ));
  ...
  const handlePickAllFound = () => {
    ...
    showConfirm(t.msgShipConfirm.replace('{n}', outboundPending.length), async () => {
      ...
      // Update reusable container tracking
      if (recycledRules.length > 0) {
        for (const shelf of outboundPending) {
          const bc = shelf.product_barcode;
          if (!bc) continue;
          const matchRule = recycledRules.find(r => bc.startsWith(r.prefix));
          if (!matchRule) continue;
          // Upsert: increment use_count
          const { data: existing } = await supabase.from('reusable_tracking').select('*').eq('barcode', bc).single().catch(() => ({ data: null }));
          if (existing) {
  ...
  return (
    <div>
      <GlobalModal modal={modal} closeModal={closeModal} t={t} />
      <Navbar
        currentUser={currentUser} userRole={userRole} handleLogout={handleLogout}
        lang={lang} setLang={setLang} currentView={currentView} setCurrentView={setCurrentView} t={t}
      />
      ...
      {currentView === 'turnover' && (
        <Turnover
          ...
          setOutboundAssignItems={setOutboundAssignItems}
      ...
      {currentView === 'mes' && (
        <MES t={t} lang={lang} currentUser={currentUser} showAlert={showAlert} />
      )}

      {currentView === 'reusable' && (
        <ReusableTracking t={t} lang={lang} showAlert={showAlert} />
      )}
